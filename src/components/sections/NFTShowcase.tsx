...
export default function NFTShowcase() {
  // Preload GLB files on component mount (client-side)
  useEffect(() => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {NFTItems.map((nft, index) => {
            const [ref, inView] = useInView({
              triggerOnce: true, // Render Canvas once when it enters view and keep it mounted
                ...
                <div className="relative">
                  <div className="w-full h-64 mb-6 rounded-xl overflow-hidden group-hover:shadow-lg group-hover:shadow-orange-500/20 transition-all">
                    {inView ? (
                      ...
                      <div className="w-full h-64 bg-gray-800" /> // Placeholder maintains layout
                    )}
                  </div>
